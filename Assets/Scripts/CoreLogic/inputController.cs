		...
		if (Time.timeScale != 0) {
			...
			if (PauseMenu != null && Win != null && PauseMenu.activeInHierarchy == false && Win.activeInHierarchy == false || TutorialFramework.tutorialActive) {
				...
				if (cursorP2.state != State.placing && cursorP2.state != State.placingLaser && cursorP2.state != State.placingMove && cursorP2.state != State.removing) {
					...
					if (Input.GetButtonDown("up_2")) { if (isValid(cursorP2.state, cursorP2.y + 1, 0, yEnd)) { cursorP2.y += 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y)); SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f);} } else if (Input.GetButton("up_2") || Input.GetAxis("xboxLeftVert2") == 1) { vertMovingP2 = true; vertDelayP2 += Time.deltaTime; if (vertDelayP2 >= delayFactor) { vertCounterP2 += (!horMovingP2 || (cursorP2.x == xEnd || cursorP2.x == 0)) ? Time.deltaTime : Time.deltaTime * diagSpeed; if (vertCounterP2 >= 1f / cursorSpeed) { if (isValid(cursorP2.state, cursorP2.y + 1, 0, yEnd)) { cursorP2.y += 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y)); } vertCounterP2 = 0f; SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f); } } } else if (Input.GetButtonDown("down_2")) { if (isValid(cursorP2.state, cursorP2.y - 1, 0, yEnd)) { cursorP2.y -= 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y));SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f); } } else if (Input.GetButton("down_2") || Input.GetAxis("xboxLeftVert2") == -1) { vertMovingP2 = true; vertDelayP2 += Time.deltaTime; if (vertDelayP2 >= delayFactor) { vertCounterP2 += (!horMovingP2 || (cursorP2.x == xEnd || cursorP2.x == 0)) ? Time.deltaTime : Time.deltaTime * diagSpeed; if (vertCounterP2 >= 1f / cursorSpeed) { if (isValid(cursorP2.state, cursorP2.y - 1, 0, yEnd)) { cursorP2.y -= 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y)); } vertCounterP2 = 0f; SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f);} } } else { vertCounterP2 = 0f; vertDelayP2 = 0f; vertMovingP2 = false; }

					if (Input.GetButtonDown("right_2")) { if (isValid(cursorP2.state, cursorP2.x + 1, 0, xEnd)) { cursorP2.x += 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y));SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f); } } else if (Input.GetButton("right_2") || Input.GetAxis("xboxLeftHor2") == 1) { horMovingP2 = true; horDelayP2 += Time.deltaTime; if (horDelayP2 >= delayFactor) { horCounterP2 += (!vertMovingP2 || (cursorP2.y == yEnd || cursorP2.y == 0)) ? Time.deltaTime : Time.deltaTime * diagSpeed; if (horCounterP2 >= 1f / cursorSpeed) { if (isValid(cursorP2.state, cursorP2.x + 1, 0, xEnd)) { cursorP2.x += 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y)); } horCounterP2 = 0f; SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f);} } } else if (Input.GetButtonDown("left_2") && (cursorP2.state != State.placeLaser && cursorP2.state != State.placeBase)) { if (isValid(cursorP2.state, cursorP2.x - 1, 0, xEnd)) { cursorP2.x -= 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y)); SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f);} } else if ((Input.GetButton("left_2") || Input.GetAxis("xboxLeftHor2") == -1) && (cursorP2.state != State.placeLaser && cursorP2.state != State.placeBase)) { horDelayP2 += Time.deltaTime; if (horDelayP2 >= delayFactor) { horMovingP2 = true; horCounterP2 += (!vertMovingP2 || (cursorP2.y == yEnd || cursorP2.y == 0)) ? Time.deltaTime : Time.deltaTime * diagSpeed; if (horCounterP2 >= 1f / cursorSpeed) { if (isValid(cursorP2.state, cursorP2.x - 1, 0, xEnd)) { cursorP2.x -= 1; moveQueueP2.Enqueue(new XY(cursorP2.x, cursorP2.y));SoundManager.PlaySound(Sounds[0].audioclip, .2f, true, .95f, 1.05f); } horCounterP2 = 0f; } } } else { horCounterP2 = 0f; horDelayP2 = 0f; horMovingP2 = false; }
				} else {
					// Cursor Rotation P2
					bool selectionMade = false;
				...
				} else if (cursorP1.state == State.placing) // in here change the sprite while choosing direction
				{
					...
					case Building.Redirecting: /*cursorSpriteP1.GetComponent<SpriteRenderer>().spriste = cursorObjP1.GetComponent<cursor1>().Sprites[3][gridManager.theGrid.directionToIndex(cursorP1.direction)];*/
						scale = .25f;
