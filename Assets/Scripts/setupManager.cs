...
public class setupManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (basePhase)
        {
            ...
            if (Input.GetKeyDown(KeyCode.E) && pOneCanBase && p1Pos.x == 0)//P1 base place
            {
                ...
                PlaceBuild(Player.PlayerOne, selection, 0, p1Pos, Direction.None);
            }
        ...
        if (basePhase == laserPhase)//pick a building that you want to place
        {
            ...
            if (noP1Direction == true) //need to select a direction for the P1 building
            {
                ...
                {
                    PlaceBuild(Player.PlayerOne, selection, i1, p1Pos, Direction.Up);
                    noP1Direction = false;
                }
                else if (Input.GetKeyDown(KeyCode.A)) //Left
                {
                    PlaceBuild(Player.PlayerOne, selection, i1, p1Pos, Direction.Left);
                    noP1Direction = false;
                }
                else if (Input.GetKeyDown(KeyCode.S)) //Down
                {
                    PlaceBuild(Player.PlayerOne, selection, i1, p1Pos, Direction.Down);
                    noP1Direction = false;
                }
                else if (Input.GetKeyDown(KeyCode.D)) //Right
                {
                    PlaceBuild(Player.PlayerOne, selection, i1, p1Pos, Direction.Right);
                    noP1Direction = false;
                }
            }
            ...
            if (noP2Direction == true) //need to select a direction for the P2 building
            {
                ...
                {
                    PlaceBuild(Player.PlayerTwo, selection, i2, p2Pos, Direction.Up);
                    noP2Direction = false;
                }
                else if (Input.GetKeyDown(KeyCode.J)) //Left
                {
                    PlaceBuild(Player.PlayerTwo, selection, i2, p2Pos, Direction.Left);
                    noP2Direction = false;
                }
                else if (Input.GetKeyDown(KeyCode.K)) //Down
                {
                    PlaceBuild(Player.PlayerTwo, selection, i2, p2Pos, Direction.Down);
                    noP2Direction = false;
                }
                else if (Input.GetKeyDown(KeyCode.L)) //Right
                {
                    PlaceBuild(Player.PlayerTwo, selection, i2, p2Pos, Direction.Right);
                    noP2Direction = false;
                }
            }
    ...
    public void PlaceBuild(Player player, Building newBuild, int val, Vector3 pos, Direction facing)
    {
        ...
        {
            print("Building placed with direction = " + facing);
